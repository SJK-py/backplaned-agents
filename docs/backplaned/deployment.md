...
- **Two Postgres databases, one server** — `bp_router` (router: users/tasks/agents/files/acl/audit) and `bp_suite` (suite: cron/suite_platform_mappings, plus two per-user pointers in user_config — the conversation AND the user's settings moved into the router's session store). Different owners, different credentials. ([`agent-suite/data-model.md`](../agent-suite/data-model.md))
...
LLM **presets** are rows in the router's `llm_presets` table, admin-managed via `/admin`; a preset's `api_key_ref` (e.g. `env://GEMINI_API_KEY`) resolves to the env secrets above. The table is seeded from a commentable JSONC catalogue (`bp_router/llm/presets_catalog.jsonc`, or `ROUTER_LLM_PRESET_CATALOG_PATH` to replace it wholesale) and **re-synced on every boot** (catalogue-managed rows are upserted, names dropped from the catalogue are pruned; admin-created presets are left alone). To **add or override** a few presets without copying the whole built-in list, drop a `deploy/presets.custom.jsonc` (copy the `.example`; gitignored, mounted into the router): it's **merged over** the bundled catalogue and a custom entry **wins on a name collision** (so e.g. repointing `default` to a local Ollama model is one entry). The re-sync is **pinned-field**: each entry overwrites only the fields it actually **lists**, and leaves any field it **omits** under operator control (an admin-UI edit to an omitted field survives). Most bundled entries list only `provider` / `concrete_model` / `api_key_ref`, so those stay catalogue-pinned while policy fields (the `min_user_level` tier gate, sampling defaults, description) are operator-owned: set them in `/admin` and they persist. To fix a policy field fleet-wide instead, **list** it in the catalogue/overlay entry — a listed field is re-applied (and re-overwritten) every boot. Use `/admin` for one-off, admin-created presets, which the sync never touches. For a hosted provider, `scripts/prod.sh` **generates the overlay** for you — repointing `default` at the chosen provider's balanced model and `default_embedding` at its embedding model — and **asks for the embedding vector width**, which it pins on `default_embedding` (Gemini `output_dimensionality` / OpenAI `dimensions`) *and* writes to `SUITE_EMBEDDING_DIM`, so the model's output width and the LanceDB schema can't drift. (It won't clobber a hand-edited overlay — see `deploy/presets.custom.jsonc.example`.) Build an image from this repo ([`Dockerfile`](../../Dockerfile)).
...
- **`api_key_ref`** (preferred) — an indirect reference resolved at call time. The only built-in scheme is `env://VAR`, which reads the *router container's* environment. The built-in provider keys already reach the container (just set the value in `deploy/.env.prod`): `env://GEMINI_API_KEY`, `env://OPENAI_API_KEY`, `env://ANTHROPIC_API_KEY` (filled by `scripts/prod.sh`). For a **custom/self-hosted** provider, pick your own var name and add it to **`deploy/.env.prod.custom`** (`MY_PROVIDER_KEY=...`) — that file is injected into the router (and `mcp_bridge`) via an `env_file`, so `env://MY_PROVIDER_KEY` works with **no `docker-compose.prod.yml` edit**. (`deploy/.env.prod`'s `--env-file` only feeds `${...}` interpolation, not the container, which is why an arbitrary new var goes in `.env.prod.custom` instead.) A missing ref resolves empty (calls 401). Only the *reference* lives in the DB/file.
