...
- **`api_key_ref`** (preferred) — an indirect reference resolved at call time. The only built-in scheme is `env://VAR`, which reads the *router container's* environment. Three vars are already passed into the container and filled by `scripts/prod.sh`: `env://GEMINI_API_KEY`, `env://OPENAI_API_KEY`, `env://ANTHROPIC_API_KEY`. Referencing **any other** var (e.g. `env://MY_LLM_KEY`) takes two edits — add it to `deploy/.env.prod` **and** to the `router` service's `environment:` block in `docker-compose.prod.yml`, because compose's `--env-file` only feeds `${...}` interpolation, not the container. Miss the second and the ref resolves empty (calls 401). Only the *reference* lives in the DB/file.
