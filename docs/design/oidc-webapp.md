...
> **Status:** design / planned. Not yet implemented. `auth_kind="oidc"`
> already exists in the data model (`bp_router/db/models.py`) and
> `reset-password` already refuses OIDC users
> (`bp_router/api/auth.py`), but no OIDC front-door flow is wired —
> `docs/backplaned/security.md` §"OIDC … planned. Not yet wired."
