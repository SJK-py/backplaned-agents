...
# Bundled preset catalogue — prod.sh reads concrete models from it BY ALIAS so
# no model strings are hardcoded here (catalog_field). An operator who replaced
# the catalogue wholesale via ROUTER_LLM_PRESET_CATALOG_PATH should pick the
# `custom` provider, which skips this lookup.
CATALOG="bp_router/llm/presets_catalog.jsonc"
...
# trimmed), so a brace-bounded awk scan is sufficient; the quoted "key": form
# only appears in real entries, never the prose comments. Empty if not found.
catalog_field() {  # catalog_field <preset-name> <field>
    awk -v want="$1" -v field="$2" '
        /{/ { name=""; val="" }
        /"name"[[:space:]]*:/ {
            n=$0; sub(/.*"name"[[:space:]]*:[[:space:]]*"/, "", n); sub(/".*/, "", n); name=n
        }
        $0 ~ ("\"" field "\"[[:space:]]*:") {
            v=$0; sub(".*\"" field "\"[[:space:]]*:[[:space:]]*\"", "", v); sub(/".*/, "", v); val=v
        }
        /}/ { if (name == want && val != "") { print val; exit } }
    ' "$CATALOG"
}
...
# the operator to merge. Uses PROVIDER/BALANCED_MODEL/EMBED_* (resolved from the
# catalogue by alias in build_env) + EMBEDDING_DIM. No-op for the custom provider.
generate_overlay() {
...
build_env() {
    ...
    # provider -> (key env var, lite/balanced/pro tier aliases, embedding alias).
    ...
    # gpt, claude-opus, …) and the embedding aliases (gemini-embedding /
    # gpt-embedding) with correct provider/model/key; only `default` and
    # `default_embedding` need repointing at the chosen provider, which
    # generate_overlay() does. The concrete models below are read FROM the
    # catalogue by alias (catalog_field) — no model strings hardcoded here.
    EMBED_ALIAS=""; EMBED_PROVIDER=""; EMBED_MODEL=""; EMBED_KEY_VAR=""; EMBED_OPT=""; BALANCED_MODEL=""
    case "$PROVIDER" in
        anthropic)
            KEY_VAR=ANTHROPIC_API_KEY
            PRESET_LITE=claude-haiku; PRESET_BALANCED=claude; PRESET_PRO=claude-opus
            EMBED_ALIAS=gpt-embedding ;;   # Anthropic has no embeddings → OpenAI
        gemini)
            KEY_VAR=GEMINI_API_KEY
            PRESET_LITE=gemini-lite; PRESET_BALANCED=gemini; PRESET_PRO=gemini-pro
            EMBED_ALIAS=gemini-embedding ;;
        openai)
            KEY_VAR=OPENAI_API_KEY
            PRESET_LITE=gpt-nano; PRESET_BALANCED=gpt; PRESET_PRO=gpt-pro
            EMBED_ALIAS=gpt-embedding ;;
        custom)
            KEY_VAR=""
    ...
    # Resolve the concrete models from the catalogue by alias (no hardcoded model
    # strings — they live in presets_catalog.jsonc). `default` takes the BALANCED
    # tier alias's model; `default_embedding` takes the EMBED_ALIAS's full
    # provider/model/key. The embedding provider_options key + the embedding key
    # var follow from the alias's provider/api_key_ref.
    if [[ "$PROVIDER" != custom ]]; then
        BALANCED_MODEL="$(catalog_field "$PRESET_BALANCED" concrete_model)"
        EMBED_PROVIDER="$(catalog_field "$EMBED_ALIAS" provider)"
        EMBED_MODEL="$(catalog_field "$EMBED_ALIAS" concrete_model)"
        local _embed_ref; _embed_ref="$(catalog_field "$EMBED_ALIAS" api_key_ref)"
        EMBED_KEY_VAR="${_embed_ref#env://}"
        [[ -n "$BALANCED_MODEL" ]] \
            || { echo "catalogue alias '$PRESET_BALANCED' has no concrete_model in $CATALOG" >&2; exit 1; }
        [[ -n "$EMBED_PROVIDER" && -n "$EMBED_MODEL" && -n "$EMBED_KEY_VAR" ]] \
            || { echo "catalogue embedding alias '$EMBED_ALIAS' incomplete in $CATALOG" >&2; exit 1; }
        # provider_options key that pins the output width for this adapter family.
        case "$EMBED_PROVIDER" in
            gemini) EMBED_OPT=output_dimensionality ;;
            openai-embeddings|openai|openai-compatible*) EMBED_OPT=dimensions ;;
            *) echo "unknown embedding provider '$EMBED_PROVIDER' for alias '$EMBED_ALIAS'" >&2; exit 1 ;;
        esac
    fi
...

    {
        ...
        echo "# Which model each preset SLOT resolves to by default. A user picks"
        echo "# their own from the web Settings page, filtered to what their tier"
        echo "# admits; the router resolves the two per call."
        echo "ROUTER_LLM_DEFAULT_PRESETS={\"pro\":\"$PRESET_PRO\",\"balanced\":\"$PRESET_BALANCED\",\"lite\":\"$PRESET_LITE\"}"
        echo "# Embeddings are deliberately NOT a slot: changing the model"
        echo "# invalidates every vector already written."
        ...
        echo "# Vision sidecar — let a TEXT-ONLY chat preset read image/PDF files via a"
        echo "# separate multimodal preset. Set the vision preset name AND list the"
        echo "# presets you've declared text-only; read_file then routes image/PDF reads"
        echo "# through the vision preset (returning text), so the main model needn't be"
        echo "# multimodal. Both empty (default) → off (each agent's own model reads files"
        echo "# and must itself accept images)."
        echo "# SUITE_DEFAULT_PRESET_MULTIMODAL="
        echo "# SUITE_TEXT_ONLY_PRESETS=[]"
        echo
        echo "# --- Channel ---"
