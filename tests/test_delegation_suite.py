...
# Operator defaults only. Nothing in this module opens a database: the
# conversation is the router's session store and the user's settings are its
# user scope, so `database_url` is never dialled.
_SETTINGS = SuiteSettings(database_url="postgresql://unused/unused")


def test_orchestrator_hand_off() -> None:
...
def test_orchestrator_message_surfaces_send_file() -> None:
...
def test_orchestrator_hand_off_fallback_on_admit_failure() -> None:
    async def _drive() -> None:
...
def test_l1_subagent_current_time_uses_user_timezone() -> None:
    ...
    async def _drive() -> None:
        ...
        out = await run_subagent(
            _Ctx(llm, _StubPeers(), store=store), LLMData(prompt="what time is it?"),
            config=_DR_CONFIG, settings=_SETTINGS,
        )
...
def test_l1_delegated_turn_normal_and_end() -> None:
    async def _drive() -> None:
        ...
        out = await run_delegated_turn(
            ctx, config=cfg, settings=_SETTINGS,
            first_turn=False,
        )
        ...
        out2 = await run_delegated_turn(
            ctx2, config=cfg, settings=_SETTINGS,
            first_turn=False,
        )
...
def test_l1_first_turn_never_hands_back() -> None:
...
def test_orchestrator_end_delegation_recap() -> None:
    async def _drive() -> None:
        store = FakeStore()
...
def test_l1_end_delegation_auto_forwards_user_message() -> None:
...
def test_l1_end_delegation_explicit_user_prompt_wins() -> None:
...
def test_l1_end_delegation_carries_queued_file() -> None:
...
def test_orchestrator_end_delegation_delivers_handback_files() -> None:
...
def test_orchestrator_end_delegation_merges_files_with_followup() -> None:
...
def test_gateway_delegated_to_maintenance() -> None:
    async def _drive() -> None:
        store = FakeStore()
        # `pool=None`: this exercises the channel engine's `delegated_to`
        # maintenance, which is entirely session-store state now. The gateway
        # still takes a pool for platform mappings; nothing here acquires it.
        gw = ChatbotGateway(
            dispatcher=None, pool=None, telegram=None,
            store=FakeChannelStore(store), settings=_SETTINGS,
        )
        core = gw._core
